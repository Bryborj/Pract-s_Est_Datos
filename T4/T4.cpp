...
public:
    ...
    void addSong(string t, string a, int d){
        ...
        if (!head) {
            head = tail = current = newSong; // 1er Nodo
        } else {
            tail->next = newSong;
            newSong->prev = tail;
            tail = newSong; // Agregar al final
        }
    }
    ...
    void nextSong(){
        if (current && current->next) {
            ...
            cout << "Fin de la playlist." << endl; // Validación requerida
        }
    }
    ...
    void previousSong(){
        if (current && current->prev) {
            ...
            cout << "Inicio de la playlist." << endl; // Validación requerida
        }
    }
    ...
    void deleteSong(){
        ...
        delete toDelete;// Evitar fugas de memoria
